...
scoreboard players set #destroyed vpcr.temp 0
execute unless block ~ ~ ~ minecraft:barrel run function vplib:custom_crafting/v1/block/table/destroyed

# Manage gui
execute if score #destroyed vpcr.temp matches 0 if block ~ ~ ~ minecraft:barrel[open=true] run function vplib:custom_crafting/v1/block/table/gui/main
