...
# Register
execute unless score #vanilla.beehive vplib.recipe matches 1.. store result score #vanilla.beehive vplib.recipe run scoreboard players add $register vplib.recipe 1
...
execute if score #recipe vplib.temp matches 0 if score #matched vplib.temp matches 1 if data storage vplib:temp {recipeInput:[{Slot:11b,id:"minecraft:honeycomb"},{Slot:12b,id:"minecraft:honeycomb"},{Slot:13b,id:"minecraft:honeycomb"}]} run scoreboard players operation #recipe vplib.temp = #vanilla.beehive vplib.recipe

# Output
execute if score #recipe vplib.temp = #vanilla.beehive vplib.recipe run data modify storage vplib:temp recipeOutput set value {id:"minecraft:beehive",Count:1b}