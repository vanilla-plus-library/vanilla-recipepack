...
# Register
execute unless score #vanilla.wooden_pickaxe vplib.recipe matches 1.. store result score #vanilla.wooden_pickaxe vplib.recipe run scoreboard players add $register vplib.recipe 1
...
execute if score #recipe vplib.temp matches 0 if score #matched vplib.temp matches 1 if data storage vplib:temp {recipeInput:[{Slot:12b,id:"minecraft:stick"},{Slot:21b,id:"minecraft:stick"}]} run scoreboard players operation #recipe vplib.temp = #vanilla.wooden_pickaxe vplib.recipe

# Output
execute if score #recipe vplib.temp = #vanilla.wooden_pickaxe vplib.recipe run data modify storage vplib:temp recipeOutput set value {id:"minecraft:wooden_pickaxe",Count:1b,tag:{Damage:0}}